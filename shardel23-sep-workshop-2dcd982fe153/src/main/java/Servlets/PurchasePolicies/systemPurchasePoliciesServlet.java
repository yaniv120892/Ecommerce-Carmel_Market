...
@WebServlet(urlPatterns = "/systemPurchasePolicies")
public class systemPurchasePoliciesServlet extends HttpServlet {
    ServiceProvider sc;

    public systemPurchasePoliciesServlet() {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            if (request.getSession().getAttribute("userID") == null) {
                ...
                List<Item> listItems  = sc.getAllItems();

                request.setAttribute("listCategories", new ArrayList<>(Arrays.asList(CategoryType.values())));
                request.setAttribute("listItems", listItems);
                RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/views/PurchasePolicies/systemPurchasePolicies.jsp");
                dispatcher.forward(request, response);
            }
        } catch (Exception e) {
